...
import { Person, Lock, HowToReg } from '@mui/icons-material';
...
const SignupPage = ({ onSignup }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('http://localhost:5000/api/register', { // Use your actual backend URL
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                onSignup(data.token); // Use the token from the backend
                navigate('/home');
            } else {
    ...
    return (
        <Grid container justifyContent="center" alignItems="center" style={{ minHeight: '100vh', background: 'linear-gradient(to bottom, #e0f7fa, #80deea)' }}>
            <Grid item xs={12} sm={8} md={6} lg={4}>
                <GradientCard>
                    <GradientCardContent>
                        <HowToReg sx={{ fontSize: 40, color: 'white', marginBottom: 2 }} />
